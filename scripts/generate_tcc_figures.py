...
def make_pipeline_overview() -> None:
    ...
    stages = [
        ...
        (8.2,  "Transforms\nlog1p + derived\n+ aug.",         PALETTE["bg2"]),
        (10.2, "Backbone\nResNet + HSDC / SWHDC",             PALETTE["lightred"]),
...
def _draw_block_diagram(ax, color, combine_box_text, output_text,
    ...
    ax.plot([hub_x, hub_x], [4.65, 1.35], color=PALETTE["edge"], lw=1.2)
    ...
    hub2_x = 5.4
    ax.plot([hub2_x, hub2_x], [4.65, 1.35], color=PALETTE["edge"], lw=1.2)
    for y_b in branch_y:
...
def make_gaussian_primitive() -> None:
    """3D anisotropic Gaussian: ellipsoid + scales + rotation + opacity."""
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection="3d")

    # Build an ellipsoid via parametric surface
    u, v = np.linspace(0, 2 * np.pi, 60), np.linspace(0, np.pi, 30)
    uu, vv = np.meshgrid(u, v)
    s = np.array([0.55, 0.30, 0.18])

    # Rotation: arbitrary, expressed via Euler angles
    theta_x, theta_y, theta_z = np.deg2rad(20), np.deg2rad(35), np.deg2rad(15)
    ...
    x = s[0] * np.cos(uu) * np.sin(vv)
    y = s[1] * np.sin(uu) * np.sin(vv)
    z = s[2] * np.cos(vv)
    pts = np.stack([x.ravel(), y.ravel(), z.ravel()], axis=0)
    pts = R @ pts
    x = pts[0].reshape(uu.shape); y = pts[1].reshape(uu.shape); z = pts[2].reshape(uu.shape)
    ax.plot_surface(x, y, z, alpha=0.45, color=PALETTE["hsdc"], edgecolor="none")

    # Principal axes (eigenvectors of Σ)
    origin = np.zeros(3)
    for i, scale in enumerate(s):
        axis_dir = R[:, i] * scale
        ax.quiver(*origin, *axis_dir, color=PALETTE["red"], lw=1.5,
                  arrow_length_ratio=0.15)
        end = axis_dir
        ax.text(*end, f"  $\\mathbf{{R}}_{{:,{i+1}}} s_{i+1}$",
                fontsize=10, color=PALETTE["red"])

    # Centre μ
    ax.scatter([0], [0], [0], color=PALETTE["edge"], s=40)
    ax.text(0, 0, -0.06, "$\\mathbf{\\mu}$", fontsize=12, color=PALETTE["edge"],
            ha="center")

    ax.set_xlabel("x"); ax.set_ylabel("y"); ax.set_zlabel("z")
    ax.set_title(r"3D Gaussian primitive: $\mathbf{\Sigma}=\mathbf{R}\,\mathrm{diag}(\mathbf{s})^2\mathbf{R}^\top$",
                 fontsize=11)
    ax.set_box_aspect((1, 1, 1))
    ax.view_init(elev=20, azim=35)

    # Annotation box
    text = "Parameters per Gaussian:\n" \
           "  centre $\\mathbf{\\mu}\\in\\mathbb{R}^3$\n" \
           "  scale $\\mathbf{s}\\in\\mathbb{R}^3_+$\n" \
           "  rotation $\\mathbf{q}\\in\\mathbb{H}\\Rightarrow\\mathbf{R}\\in\\mathrm{SO}(3)$\n" \
           "  opacity $\\alpha\\in[0,1]$\n" \
           "  SH colour $\\mathbf{c}_{\\ell m}$"
    ax.text2D(0.02, 0.98, text, transform=ax.transAxes, fontsize=9,
              va="top", ha="left",
              bbox=dict(facecolor=PALETTE["bg"], edgecolor=PALETTE["edge"],
                        linewidth=0.6, pad=0.4))
...
def make_latitude_distortion() -> None:
    ...
    rho = 1.0 / np.sin(phi)
    ax = axes[0]
    ax.plot(np.rad2deg(phi), rho, color=PALETTE["hsdc"], lw=2,
    ...
    # Right: schematic ERP showing receptive fields at equator vs near pole.
    # The image is stretched horizontally to mimic the ERP aspect, and the
    # explanatory labels are placed in the margins above/below the imshow so
    # they never overlap the title or the rectangles.
    ax = axes[1]
    H, W = 10, 20
    ax.imshow(np.tile(np.linspace(0.8, 1.0, W), (H, 1)), cmap="Greys",
              extent=(0, W, H, 0), vmin=0, vmax=1, aspect="auto")
    # Equator RF — sits on the H/2 row
    rf_eq = Rectangle((6, H / 2 - 0.5), 3, 1, facecolor="none",
                      edgecolor=PALETTE["hsdc"], lw=2)
    ax.add_patch(rf_eq)
    ax.annotate("equator RF\n(geodesic $\\approx$ 3 px)",
                xy=(7.5, H / 2 + 0.5), xytext=(2, H - 0.5),
                ha="left", va="bottom", fontsize=8, color=PALETTE["hsdc"],
                arrowprops=dict(arrowstyle="-", color=PALETTE["hsdc"], lw=0.6))
    # Near-pole RF — same pixel width but covers a much smaller geodesic span
    rf_pole = Rectangle((9, 1.0), 3, 1, facecolor="none",
                        edgecolor=PALETTE["swhdc"], lw=2)
    ax.add_patch(rf_pole)
    ax.annotate("near-pole RF\n(geodesic $\\ll$ 3 px)",
                xy=(10.5, 1.0), xytext=(15.0, -0.6),
                ha="left", va="bottom", fontsize=8, color=PALETTE["swhdc"],
                annotation_clip=False,
                arrowprops=dict(arrowstyle="-", color=PALETTE["swhdc"], lw=0.6))
    ax.set_xlim(0, W); ax.set_ylim(H, -1.8)
    ax.set_xticks([]); ax.set_yticks([])
    for s in ax.spines.values():
        s.set_visible(True); s.set_color("#999"); s.set_linewidth(0.6)
    ax.set_title("Effective receptive field shrinks near the poles",
                 pad=14)
...
def make_augmentation_samples() -> None:
    """Original + each augmentation on a real chair ERP."""
    erp = _load_erp("chair", "train", "chair_0001")
    ...
    for ax, (title, img) in zip(axes.flat, panels):
        ...
        ax.set_title(title, fontsize=10)
        ax.set_xticks([]); ax.set_yticks([])
    fig.suptitle("Single-sample augmentation primitives on a chair RF-ERP",
                 fontsize=12, y=1.02)
    plt.tight_layout()
